...
* +: Confirmation de suppression de stream depuis le panel
* Fix: Add preference for the xhr timeout duration
* /!\: clipboard for prompt if no direct clipboard access
...
* i: Changed Request function to be more alike Firefox API (Request().get)
* i: loadJS now using Promise and no callback anymore
...
* i: Starting to test Webextension in Firefox, with needed changes
* i: Moved the loadPreferences function for option and panel to option-api
* i: SVG pictures are now rendered into canvas using Image and not canvg
* i: Preferences now using chrome local storage area, with a copy of the preferences in a variable to avoid async
...
* +: Grouped timing for checkLives streams
* +: Stream setting to ignore streams on the icon
* +: Chrome API notification, contextMessage to say it's notification from "Live notifier"
* +: Theme on the option page
* +: Streams configured with unsupported / not load website are now shown in the panel
* +: Add a custom timeout in Request (30s)
* +: Adding stream with context menu get redirected url (using meta equiv)
* +: Pressing shift when clicking on the button to import from file or sync will merge instead of replacing
* +: min and max value support for integer preferences
* -: Dropped Web notifications support
* Fix: Unsuccessful channel list requests
* Fix: Online icon badge
* Fix: Stream setting not displayed in option page
* Fix: Beam importation
* Fix: Some French translations
* Fix: Using onClick event again on notification without button
